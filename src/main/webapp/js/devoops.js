...
function LoadCalendarScript(callback){
	function LoadFullCalendarScript(){
		if(!$.fn.fullCalendar){
			$.getScript(srpp+'/plugins/fullcalendar/fullcalendar.js', callback);
		}
		else {
	...
	if (!$.fn.moment){
		$.getScript(srpp+'/plugins/moment/moment.min.js', LoadFullCalendarScript);
	}
	else {
...
function LoadLeafletScript(callback){
	if (!$.fn.L){
		$.getScript(srpp+'/plugins/leaflet/leaflet.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadTimePickerScript(callback){
	if (!$.fn.timepicker){
		$.getScript(srpp+'/plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadBootstrapValidatorScript(callback){
	if (!$.fn.bootstrapValidator){
		$.getScript(srpp+'/plugins/bootstrapvalidator/bootstrapValidator.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSelect2Script(callback){
	if (!$.fn.select2){
		$.getScript(srpp+'/plugins/select2/select2.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadDataTablesScripts(callback){
	function LoadDatatables(){
		$.getScript(srpp+'/plugins/datatables/jquery.dataTables.js', function(){
			$.getScript(srpp+'/plugins/datatables/ZeroClipboard.js', function(){
				$.getScript(srpp+'/plugins/datatables/TableTools.js', function(){
					$.getScript(srpp+'/plugins/datatables/dataTables.bootstrap.js', callback);
				});
			});
		});
	}
...
function LoadFineUploader(callback){
	if (!$.fn.fineuploader){
		$.getScript(srpp+'/plugins/fineuploader/jquery.fineuploader-5.0.5.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadXChartScript(callback){
	function LoadXChart(){
		$.getScript(srpp+'/plugins/xcharts/xcharts.min.js', callback);
	}
	function LoadD3Script(){
		if (!$.fn.d3){
			$.getScript(srpp+'/plugins/d3/d3.min.js', LoadXChart)
		}
		else {
			LoadXChart();
		}
...
function LoadFlotScripts(callback){
	function LoadFlotScript(){
		$.getScript(srpp+'/plugins/flot/jquery.flot.js', LoadFlotResizeScript);
	}
	function LoadFlotResizeScript(){
		$.getScript(srpp+'/plugins/flot/jquery.flot.resize.js', LoadFlotTimeScript);
	}
	function LoadFlotTimeScript(){
		$.getScript(srpp+'/plugins/flot/jquery.flot.time.js', callback);
	}
	if (!$.fn.flot){
		LoadFlotScript();
	}
...
function LoadMorrisScripts(callback){
	function LoadMorrisScript(){
		if(!$.fn.Morris){
			$.getScript(srpp+'/plugins/morris/morris.min.js', callback);
		}
		else {
			if (callback && typeof(callback) === "function") {
				callback();
	...
	if (!$.fn.raphael){
		$.getScript(srpp+'/plugins/raphael/raphael-min.js', LoadMorrisScript);
	}
	else {
		LoadMorrisScript();
	}
...
function LoadAmchartsScripts(callback){
	function LoadAmchartsScript(){
		$.getScript(srpp+'/plugins/amcharts/amcharts.js', LoadFunnelScript);
	}
	function LoadFunnelScript(){
		$.getScript(srpp+'/plugins/amcharts/funnel.js', LoadSerialScript);
	}
	function LoadSerialScript(){
		$.getScript(srpp+'/plugins/amcharts/serial.js', LoadPieScript);
	}
	function LoadPieScript(){
		$.getScript(srpp+'/plugins/amcharts/pie.js', callback);
	}
	if (!$.fn.AmCharts){
		LoadAmchartsScript();
	}
...
function LoadChartistScripts(callback){
	function LoadChartistScript(){
		$.getScript(srpp+'/plugins/chartist/chartist.min.js', callback);
	}
	if (!$.fn.Chartist){
		LoadChartistScript();
	}
...
function LoadSpringyScripts(callback){
	function LoadSpringyScript(){
		$.getScript(srpp+'/plugins/springy/springy.js', LoadSpringyUIScript);
	}
	function LoadSpringyUIScript(){
		$.getScript(srpp+'/plugins/springy/springyui.js', callback);
	}
	if (!$.fn.Springy){
		LoadSpringyScript();
	}
...
function LoadFancyboxScript(callback){
	if (!$.fn.fancybox){
		$.getScript(srpp+'/plugins/fancybox/jquery.fancybox.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadKnobScripts(callback){
	if(!$.fn.knob){
		$.getScript(srpp+'/plugins/jQuery-Knob/jquery.knob.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
function LoadSparkLineScript(callback){
	if(!$.fn.sparkline){
		$.getScript(srpp+'/plugins/sparkline/jquery.sparkline.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
		...
		yaxes: [
			{ min: 0 },
			{
				...
				tickFormatter: function (value, axis) {
					return value.toFixed(axis.tickDecimals) + "鈧�";
				}
			}
		],
		legend: { position: "sw" }
...
function TestTable3(){
	$('#datatable-3').dataTable( {
		...
		"oTableTools": {
			"sSwfPath": srpp+"/plugins/datatables/copy_csv_xls_pdf.swf",
			"aButtons": [
				"copy",
				"print",
				{
...
function LoadTestMap(){
	$.getJSON("http://www.telize.com/geoip?callback=?",
		function(json) {
			var osmap = new OpenLayers.Layer.OSM("OpenStreetMap");//褋芯蟹写邪薪懈械 褋谢芯褟 泻邪褉褌褘
			...
			var osmap1 = new OpenLayers.Layer.OSM("OpenStreetMap");//褋芯蟹写邪薪懈械 褋谢芯褟 泻邪褉褌褘
	...
	$.getJSON("http://www.telize.com/geoip?callback=?",
		function(json) {
			...
			L.tileLayer('http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
				attribution: 'Map data &copy; <a href="http://openstreetmap.org">OpenStreetMap</a> contributors, <a href="http://creativecommons.org/licenses/by-sa/2.0/">CC-BY-SA</a>, Imagery 漏 <a href="http://mapbox.com">Mapbox</a>',
				maxZoom: 18
			}).addTo(map);
...
/*-------------------------------------------
Function for search menu (index.jsp)
---------------------------------------------*/
//
//返回搜索的menu数组(查询字段，菜单数据)
//
function searchMenu(search,menuData){
	if(search==''){return menuData;};
	var newMenu=[];
	for(var i=0;i<menuData.length;i++){
		var menu={
				icon:menuData[i].icon,
				menuName:menuData[i].menuName,
				children:[],
				hasChild:(menuData[i].children==undefined||menuData[i].children.length==0?false:true),
				menuUri:menuData[i].menuUri
		};
		if(menuData[i].children!=undefined){
			menu.children=menuData[i].children.filter(function(item){return item.menuName.indexOf(search)>-1});
		};
		//判断该以一级菜单 名字是否满足条件  或者  有children存在
		if(menu.menuName.indexOf(search)>-1||menu.children.length>0){
			newMenu.push(menu);
		};
	}
	return newMenu;
...
//根据传入菜单数据，生成相应的html
//
function renderMenu(search,menuData){
	var html="";
	var replace='<span class="searchString">'+search+'</span>';
	for(var i=0;i<menuData.length;i++){
		var menu=menuData[i];
		menu.children=menu.children?menu.children:[];
		if(menu.hasChild||menu.children.length>0){
			var html1=	'<li class="dropdown">'+
						'<a href="#" class="dropdown-toggle active-parent active">'+
						'<span class="'+menu.icon+'" aria-hiddenx=true ></span>'+
						'<span class="hidden-xs"> '+menu.menuName.replace(search,replace)+'</span>'+
						'</a>'+
			...
			for(var j=0;j<menu.children.length;j++){
				childHtml+=	'<li>'+
							'<a class="ajax-link" href="'+menu.children[j].menuUri+'">'+menu.children[j].menuName.replace(search,replace)+'</a>'+
							'</li>';
			}
			html+=html1+childHtml+html2;
		}else{
			html+=	'<li class="sub-menu">'+
						'<a class="ajax-link" href="'+menu.menuUri+'"  >'+
