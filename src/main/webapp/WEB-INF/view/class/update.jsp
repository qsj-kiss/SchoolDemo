...
<div class="layui-form" lay-filter="layuiadmin-form-order" id="layuiadmin-form-order" style="padding: 20px 30px 0 0;">
    <!-- 内容主体区域 -->
    <h1 style="padding: 15px;">修改班级</h1>
    <form class="layui-form" action="" lay-filter="example">
    <input id="clsId" name="clsId" style="display: none" value="${class.clsId}">
        <div class="layui-form-item">
            <label class="layui-form-label">班级名称</label>
            <div class="layui-input-inline">
                <input type="text" name="clsName" value="${class.clsName}" placeholder="请输入班级名称" autocomplete="off" class="layui-input">
            </div>
        </div>
        ...
        <div class="layui-form-item layui-form-text">
            ...
            <div class="layui-input-block">
                <textarea placeholder="请输入班级备注" value="${class.clsRemark}" class="layui-textarea" name="clsRemark">${class.clsRemark}</textarea>
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-input-block">
...
<script>
  layui.use(['form', 'layedit', 'laydate','upload'], function(){
    ...
    function selected6(){
      $("#clsChinese").each(function() {
        $(this).children("option").each(function() {
          if (this.value == ${class.clsChinese}) {
    ...
    function selected5(){
      $("#clsPhysical").each(function() {
        $(this).children("option").each(function() {
          if (this.value == ${class.clsPhysical}) {
            $(this).attr("selected","selected");
            console.log(this.value)
          }
        });
    ...
    function selected3(){
      $("#clsEnglish").each(function() {
        $(this).children("option").each(function() {
          if (this.value == ${class.clsEnglish}) {
            $(this).attr("selected","selected");
            console.log(this.value)
          }
        });
    ...
    function selected2(){
      $("#clsMath").each(function() {
        $(this).children("option").each(function() {
          if (this.value == ${class.clsMath}) {
            $(this).attr("selected","selected");
            console.log(this.value)
          }
        });
    ...
    form.on('submit(demo1)', function(data){
      $.post('/class/updateClass',data.field,function (data) {
